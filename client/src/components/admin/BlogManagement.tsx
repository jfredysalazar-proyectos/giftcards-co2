      ...
      {!isCreating ? (
        ...
        <Card className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Fecha de Publicación {formData.published ? "(Programada/Real)" : "(Opcional)"}
              </label>
              <input
                type="datetime-local"
                value={formData.publishedAt ? new Date(new Date(formData.publishedAt).getTime() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16) : ""}
                onChange={(e) =>
                  setFormData({ ...formData, publishedAt: e.target.value ? new Date(e.target.value) : null })
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-purple-500 focus:border-purple-500"
              />
              <p className="text-xs text-gray-500 mt-1">
                Si dejas este campo vacío y marcas "Publicar", se usará la fecha actual.
              </p>
            </div>
